...
/**
 * 
 * AJOUTER CompétencesInfos POUR DES INFOS SUPP SUR DES COMPETENCES (COMME +2 A UNE COMP DE CHOIX)
 * 
 * 
 * 
 * 
...
function CreationPersonnageAvancee() {
    ...
    /*PRINCIPALES*/
    const [idPersonnage, setIdPersonnage] = useState(null);
    ...
    useEffect(() => {
        const dataSauvegardee = localStorage.getItem('personnage_wrathandglory');
        if (dataSauvegardee) {
            const fullData = JSON.parse(dataSauvegardee);

            // CORRECTION : On accède à la clé Personnages_WrathAndGlory
            const perso = fullData.Personnages_WrathAndGlory;
            ...
            if (perso) {
                ...
                /*PRINCIPALES*/
                setIdPersonnage(perso.Id || 0);
                setNom(perso.Nom || "");
                setTier(perso.Tier || 0);
                setPXDebut(perso.PXDebut || 0);
                setPXRestant(perso.PXRestant || 0);
                setRichesse(perso.Richesse || 0);
                setEspeceSelectionnée(perso.Espèce || "");
                setFactionSelectionnée(perso.Faction || "");
                setSousFactionSelectionnée(perso.SousFaction || "");
                setHistorique(perso.Historique || "");
                setMotsCles(perso.MotsClés || "");
                setAptitudes(perso.Aptitudes || "");
                setEquipement(perso.Equipement || "");
                setBlessureGrave1(perso.BlessuresGraves1 || "");
                setBlessureGrave2(perso.BlessuresGraves2 || "");
                setBlessureGrave3(perso.BlessuresGraves3 || "");
                setBlessureGrave4(perso.BlessuresGraves4 || "");
                setBlessureGrave5(perso.BlessuresGraves5 || "");
                setBlessureGrave6(perso.BlessuresGraves6 || "");
                setBlessuresGuerre(perso.BlessuresGuerre || "");
                setMutations(perso.Mutations || "");
                setBlessures(perso.Bléssures || "");
                setStress(perso.Stress || "");
                setRang(perso.Rang || 0);


                /*ATTRIBUTS*/
                setForce(perso.Force || 1);
                setEdurance(perso.Endurance || 1);
                setAgilité(perso.Agilité || 1);
                setInitiative(perso.Initiative || 1);
                setForceMentale(perso.ForceMentale || 1);
                setIntelligence(perso.Intelligence || 1);
                setSociabilité(perso.Sociabilité || 1);

                /*BONUS ATTRIBUTS*/
                setBonusForce(perso.BonusForce || 0);
                setBonusEndurance(perso.BonusEndurance || 0);
                setBonusAgilité(perso.BonusAgilité || 0);
                setBonusInitiative(perso.BonusInitiative || 0);
                setBonusForceMentale(perso.BonusForceMentale || 0);
                setBonusIntelligence(perso.BonusIntelligence || 0);
                setBonusSociabilité(perso.BonusSociabilité || 0);

                /*COMPETENCES*/
                setAthlétisme(perso.Athlétisme || 0);
                setCapacitéDeCombat(perso.CapacitéDeCombat || 0);
                setCapacitéDeTir(perso.CapacitéDeTir || 0);
                setCommandement(perso.Commandement || 0);
                setConnaissance(perso.Connaissance || 0);
                setDuperie(perso.Duperie || 0);
                setFurtivité(perso.Furtivité || 0);
                setIntimidation(perso.Intimidation || 0);
                setInvestigation(perso.Investigation || 0);
                setMaîtrisePsychique(perso.MaîtrisePsychique || 0);
                setMedicae(perso.Medicae || 0);
                setPersuasion(perso.Persuasion || 0);
                setPilotage(perso.Pilotage || 0);
                setPsychologie(perso.Pshychologie || 0);
                setRuse(perso.Ruse || 0);
                setSurvie(perso.Survie || 0);
                setTechnomaîtrise(perso.Technom0îtrise || 0);
                setVigilance(perso.Vigilance || 0);

                /*TABLEAUX*/
                setPouvoirPsychiquePossedees(perso.PouvoirsPsychiques || []);
                setArmePossedees(perso.Armes || []);
                setArmurePossedees(perso.Armures || []);
                setTalentsPossedees(perso.Talents || []);
                setCorruptionData(perso.corruption || Array(5).fill(null).map(() => Array(6).fill(false)));
            }
        }
    }, []);
    ...
    // Pour DDL espèce
    useEffect(() => {
        if (especeSelectionnée && !selectedItem) {
            const found = ListeEspeces.find(e => e.Espèce === especeSelectionnée);
            if (found) setSelectedItem(found.Id);
        }
    }, [especeSelectionnée, ListeEspeces]);

    // Pour DDL faction
    useEffect(() => {
        if (factionSelectionnée && !selectedItemFaction) {
            const found = Faction.find(e => e.faction === factionSelectionnée || e.Faction === factionSelectionnée);
            if (found) setSelectedItemFaction(found.id || found.Id);
        }
    }, [factionSelectionnée, Faction]);

    // Pour DDL sous faction
    useEffect(() => {
        if (sousFactionSelectionnée && !selectedItemSousFaction) {
            const found = SousFaction.find(e => e.sousfaction === sousFactionSelectionnée || e.SousFaction === sousFactionSelectionnée);
            if (found) setSelectedItemSousFaction(found.id || found.Id);
        }
    }, [sousFactionSelectionnée, SousFaction]);

    //modification personnage
    const SauvegarderModifications = async () => {
        if (!idPersonnage) {
            window.alert("Erreur : Impossible de trouver l'ID du personnage.");
            return;
        }
        ...
        const donneesMisesAJour = {
            ...
            Tier: tier,
            Rang: rang,
            Espèce: especeSelectionnée,
            Faction: factionSelectionnée,
            SousFaction: sousFactionSelectionnée,
            MotsClés: motsCles,
            ...
            Agilité: agilité,
            BonusAgilité: bonusAgilité,
            Initiative: initiative,
            BonusInitiative: bonusInitiative,
            ForceMentale: forceMentale,
            BonusForceMentale: bonusForceMentale,
            Intelligence: intelligence,
            BonusIntelligence: bonusIntelligence,
            Sociabilité: sociabilité,
            BonusSociabilité: bonusSociabilité,

            /* COMPÉTENCES */
            Athlétisme: athlétisme,
            CapacitéCombat: capacitéDeCombat,
            CapacitéTir: capacitéDeTir,
            Commandemant: commandement,
            Connaissance: connaissance,
            Duperie: duperie,
            Furtivité: furtivité,
            Intimidation: intimidation,
            Investigation: investigation,
            MaîtrisePsychique: maîtrisePsychique,
            Medicae: medicae,
            ...
            Ruse: ruse,
            Survie: survie,
            Technomaîtrise: technomaîtrise,
            Vigilance: vigilance,

            /* LISTES ET ÉQUIPEMENT */
            TalentsPossedees: TalentsPossedees,
            Aptitudes: aptitudes,
            pouvoirPsychiquePossedees: pouvoirPsychiquePossedees,
            ArmePossedees: ArmePossedees,
            ArmurePossedees: ArmurePossedees,
        ...
        try {
            ...
            const res = await axios.patch(`${API_URL}/API/API_Personnages_WrathAndGlory/${idPersonnage}`, donneesMisesAJour);

            if (res.status === 200) {
                window.alert("Modifications sauvegardées avec succès !");

                // Mettre à jour le localStorage avec les nouvelles données
                const personnageMisAJour = res.data.Personnage;
                localStorage.setItem('personnage_wrathandglory', JSON.stringify(personnageMisAJour));

                console.log("Personnage mis à jour:", personnageMisAJour);
            }
        } catch (err) {
            console.error("Erreur lors de la sauvegarde :", err.response?.data || err.message);
            window.alert("Erreur lors de la sauvegarde : " + (err.response?.data?.message || err.message));
        }
    };
    ...
    return (
        <>
            ...
            <div className="Container-Creation-Avancee-Global">
                <div style={{ margin: 'auto', display: 'block', width: 'fit-content', top: '5rem', backgroundColor: 'black', padding: '1rem', borderRadius: '20px', border: '2px solid hsl(0 0% 60%)', zIndex: '999999' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'row' }}>
                    <div style={{ width: '50%', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <div className="Input" style={{ position: "relative" }}>
                            ...
                            <div className='dropdown' style={{ position: "absolute", left: '8rem', top: '-0.5rem', zIndex: '9999' }}>
                                <div className='dropdown-header' onClick={toggleDropdown} style={{ height: '1rem' }}>
                                    {selectedItem ? ListeEspeces.find(item => item.Id === selectedItem || item.Espèce === especeSelectionnée).Espèce : "Toutes les Espèces"}
                                    ...
                                    <div className="dropdown-body open">
                                        {ListeEspeces.map(item => (
                                            <div className="dropdown-item" onClick={() => handleItemClick(item.Id)} key={item.Id + "_Espèce"}>
                                                <span className={`dropdown-item-dot ${(item.Id === selectedItem || item.Espèce === especeSelectionnée) && 'selected'}`}>• </span>
                                                {item.Espèce}
                                            </div>
                                        ))}
                                    </div>
                        ...
                        <div className="Input">
                            ...
                            <input className="Input-Personage" type="text" style={{ width: '100%' }} value={motsCles} onChange={(e) => setMotsCles(e.target.value)} />
                        </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'row', gap: '5rem' }}>
                    ...
                    <div style={{ width: '40%', margin: 'auto' }}>
                        ...
                        <textarea style={{ resize: 'none', width: '100%', height: '20rem', padding: '2rem' }} value={aptitudes} onChange={(e) => setAptitudes(e.target.value)}></textarea>
                    </div>
                </div>
                            ...
                            <tbody>
                                {
                                    pouvoirPsychiquePossedees.map(pouvPsy => (
                                        ...
                                        <tr style={{ position: 'relative' }} key={pouvPsy.Id + "_PouvoirsPsychiques"}>
                ...
                <div style={{ backgroundColor: 'black', border: '2px solid hsl(0 0% 60%)', borderRadius: '20px', minHeight: '20vh', width: '80vw', margin: 'auto', padding: '1rem' }}>
                    <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', width: '100%', gap: '1rem' }}>
                        <div className="Bouton-Pouvoirs-Psychiques" onClick={() => { setSelectedListeArmure(ListeArmesImpériales); AfficheDivSelectArme() }}><label>Armes Impériales</label></div>
                        <div className="Bouton-Pouvoirs-Psychiques" onClick={() => { setSelectedListeArmure(ListeArmesAeldari); AfficheDivSelectArme() }}><label>Armes Aeldari</label></div>
                            ...
                            <tbody>
                                {
                                    ArmePossedees.map(arme => (
                                        ...
                                        <tr style={{ position: 'relative' }} key={arme.Id + "_Arme" }>
                                            <td style={{ borderTop: '2px solid hsl(0 0% 60%)', borderRight: '2px solid hsl(0 0% 60%)' }}>
                            ...
                            <tbody>
                                {
                                    ArmurePossedees.map(Armure => (
                                        ...
                                        <tr style={{ position: 'relative' }} key={Armure.Id + "_Armure"}>
                                            <td style={{ borderTop: '2px solid hsl(0 0% 60%)', borderRight: '2px solid hsl(0 0% 60%)' }}>
                ...
                <button
                    className="Bouton-Enregistrer-Personnage-Avancée"
